...
def ensure_lemonade_server(args) -> bool:
    ...
    # 서버 실행 명령 후보 (PATH에 있는 것을 순서대로 시도)
    ...
    if exe is None:
        print(f"[로컬][오류] Lemonade 실행파일을 찾지 못했습니다 (시도: {', '.join(candidates)}). "
              f"PATH를 확인하거나 --local-serve-cmd로 경로를 지정하세요. "
              f"수동으로 'lemonade-server-dev serve'를 먼저 켜두면 그 서버를 재사용합니다.")
        return False

    serve_args = [exe, "serve"]
    if port != LEMONADE_DEFAULT_PORT:
        serve_args += ["--port", str(port)]
    print(f"[로컬] Lemonade 서버 자동 기동 중... ({exe} serve, 포트 {port})")
    try:
        # 콘솔 출력은 조용히(로그 파일로), 별도 프로세스 그룹으로 띄워 메인 종료 시 남지 않게 함
        ...
        print(f"[로컬][오류] 서버 기동 실패: {e}")
        return False

    # 서버가 응답할 때까지 대기 (모델 백엔드 초기화에 시간이 걸릴 수 있음)
    print("[로컬] 서버 준비 대기 중 (최대 120초)...")
    for _ in range(120):
        if is_lemonade_up(port):
            print("[로컬] 서버 준비 완료")
            return True
        if _LEMONADE_PROC.poll() is not None:
            print(f"[로컬][오류] 서버 프로세스가 조기 종료됨. 로그 확인: {log_path}")
            return False
        time.sleep(1)
    ...
    def next_alive_index(start: int) -> int | None:
        ...
        if local:
            if not local_started["done"]:
                print("  [폴백] 클라우드 API 전부 소진 -> 로컬 NPU로 전환 시도")
