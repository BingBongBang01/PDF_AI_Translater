...
from pdf_engine.config.settings import DEFAULT_LOCAL_RUNTIME, DEFAULT_MODELS, get_runtime_spec, resolve_local_model_for_device
...
def _try_start_one(exe: str, extra_args: list[str], port: int, args, runtime: str) -> bool:
    ...
    get_logger().log(f"[로컬] 기동 시도: {' '.join(serve_args)} (포트 {port})")

    # 한국어 Windows에서 자식 프로세스의 stdout이 파일로 리다이렉트되면 기본 인코딩이 cp949가 되어
    # (구버전) lemonade-server-dev 내부 get_logger().log()의 유니코드 문자(예: '•')에서 UnicodeEncodeError로 즉시 죽는다.
    ...
    try:
        ...
        get_logger().log(f"[로컬][오류] '{exe}' 실행 자체 실패: {e}")
        return False

    get_logger().log("[로컬] 서버 준비 대기 중 (최대 180초, 첫 실행은 모델 초기화로 오래 걸릴 수 있음)...")
    for _ in range(180):
        if is_local_runtime_up(port, runtime):
            get_logger().log("[로컬] 서버 준비 완료")
            return True
        if _LOCAL_PROC.poll() is not None:
            get_logger().log(f"[로컬][오류] '{exe}' 프로세스가 조기 종료됨 (exit code {_LOCAL_PROC.returncode}).")
            get_logger().log(f"[로컬] --- 서버 로그 (마지막 25줄) ---")
            get_logger().log(_tail_file(log_path))
            get_logger().log(f"[로컬] --- 로그 끝 (전체: {log_path}) ---")
            _LOCAL_PROC = None
            return False
        time.sleep(1)
    get_logger().log(f"[로컬][오류] 서버가 제한 시간 내에 준비되지 않음. 로그 확인: {log_path}")
    return False
    ...
    try:
        with urllib.request.urlopen(req, timeout=120) as r:
            ...
            if ok:
                ...
                get_logger().log(f"[로컬][경고] 모델 로드 응답 코드 {r.status} (기존 로드 설정으로 계속 진행)")
            return ok
    except Exception as e:
        ...
        try:
            ...
            with urllib.request.urlopen(alt_req, timeout=120) as r:
                if r.status == 200:
                    get_logger().log(f"[로컬] 모델 로드 완료 (대체 방식: {list(alt_dict.keys())[-1]})")
                    return True
        except Exception:
            pass
        get_logger().log(f"[로컬][경고] 명시적 컨텍스트 로드 실패, 기존 로드 상태로 계속 진행: {e}")
        return False
...
def ensure_local_runtime(args) -> bool:
    ...
    if is_local_runtime_up(port, runtime):
        get_logger().log(f"[로컬] 이미 실행 중인 {spec['label']} 감지 (포트 {port}) -> 재사용")
        _after_up()
        return True
    ...
    if not found:
        ...
        get_logger().log(f"[로컬][오류] {spec['label']} 실행파일을 찾지 못했습니다 (시도: {names}).")
    ...
    for exe, extra in found:
        ...
        if is_local_runtime_up(port, runtime):
            get_logger().log(f"[로컬] 서버가 포트 {port}에 떠 있음 -> 재사용")
            _after_up()
            return True

    get_logger().log(f"[로컬][오류] 모든 기동 명령({', '.join(c[0] for c in found)})이 실패했습니다.")
    get_logger().log(f"[로컬] 위 서버 로그를 확인하세요. 흔한 원인:")
    get_logger().log(f"        - 포트 {port} 이미 사용 중 -> 그 서버를 재사용해야 하는데 감지 실패")
    get_logger().log(f"        - 메모리 부족(다른 앱, 특히 브라우저를 닫고 재시도)")
    get_logger().log(f"        - 해결이 안 되면 수동으로 {spec['label']}를 먼저 켜두고 이 스크립트를 다시 실행")
    return False
...
def shutdown_local_runtime():
    ...
    if _LOCAL_PROC is not None:
        get_logger().log("[로컬] 자동 기동한 로컬 서버 종료")
        try:
...
def local_presets_for(model_name: str) -> dict:
    ...
    if bits is None:
        bits = 4
        get_logger().log(f"[로컬] 모델명에서 양자화 비트를 감지하지 못함 -> 4비트 프리셋 사용 (FLM 기본 Q4 가정)")
...
def timed_input(prompt: str, timeout: float) -> str | None:
    ...
    """
    # 주의: 여긴 대화형 콘솔 입력이라 Logger가 아니라 print를 직접 써야 한다.
    # (Logger.log()는 (message, level) 시그니처라 end/flush를 받지 못해 TypeError로 죽었고,
    #  인자 없는 get_logger().log()도 마찬가지였다 - CLI에서 모델 선택 메뉴가 뜨는 순간
    #  번역이 시작도 못 하고 중단됐다.)
    ...
    if os.name == "nt":
        ...
        try:
            ...
            rlist = []          # stdin이 없는 환경(GUI/EXE)에서 select가 실패할 수 있다
        if rlist:
...
def fetch_local_models(port: int) -> list[str]:
    ...
    try:
        ...
        get_logger().log(f"[로컬][경고] 모델 목록 조회 실패: {e}")
        return []
...
def fetch_loaded_local_model(port: int) -> str | None:
    ...
    try:
        ...
        get_logger().log(f"[로컬][경고] 로드된 모델 감지 실패: {e}")
    return None


def prepare_local_model(args) -> None:
    """로드된 모델을 최우선 기본값으로 선택하고, 필요할 때만 모델을 로드한다."""
    # 사용자가 모델을 명시했으면(GUI의 NPU/GPU 모델 드롭다운 = --model-local-npu/-gpu)
    # 그 선택이 최우선이다. 예전엔 서버에 이미 로드돼 있던 다른 모델로 무조건 덮어써서,
    # GUI에서 고른 모델과 실제로 로드하는 모델이 어긋났다.
    explicit = (getattr(args, "model_local", None) or getattr(args, "model_local_npu", None)
                or getattr(args, "model_local_gpu", None))
    loaded_model = fetch_loaded_local_model(args.local_port)
    if explicit:
        if not args.model_local:
            args.model_local = explicit
        if loaded_model and loaded_model != args.model_local:
            get_logger().log(f"[로컬] 서버에 {loaded_model}이(가) 로드돼 있지만 "
                             f"사용자가 지정한 {args.model_local}(으)로 전환합니다.")
    elif loaded_model:
        get_logger().log(f"[로컬] 현재 로드된 모델 감지: {loaded_model} -> 최우선 기본값")
        args.model_local = loaded_model
    selected = choose_local_model(args)
    args.model_local = selected
    if loaded_model == selected:
        get_logger().log(f"[로컬] 이미 로드된 모델 재사용: {selected}")
        return
    load_local_model(args)
...
def choose_local_model(args) -> str:
    ...
    if not models:
        get_logger().log(f"[로컬] 모델 목록을 못 가져와 기본 모델로 진행: {default_model}")
        return default_model
    ...
    get_logger().log(f"[로컬] 사용 가능한 모델 (기본값: 1번 {default_model}):")
    for i, m in enumerate(models, 1):
        marker = " (기본값)" if m == default_model else ""
        get_logger().log(f"  {i}. {m}{marker}")
    ...
    if ans:
        try:
            idx = int(ans)
            if 1 <= idx <= len(models):
                get_logger().log(f"[로컬] 선택됨: {models[idx - 1]}")
                return models[idx - 1]
        except ValueError:
            # 번호가 아니라 모델명을 직접 친 경우도 허용
            if ans in models:
                get_logger().log(f"[로컬] 선택됨: {ans}")
                return ans
        get_logger().log(f"[로컬] 잘못된 입력 '{ans}' -> 기본값으로 진행")
    else:
        get_logger().log(f"[로컬] 입력 없음 -> 기본값으로 진행: {default_model}")
    return default_model