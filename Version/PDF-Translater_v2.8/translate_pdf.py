...
def main():
    ...
    # 손상/0페이지 PDF가 최종 파일로 남지 않도록 임시 파일에 저장 후 검증하고 원자적으로 교체한다.
    ...
    try:
        ...
        if check.page_count <= 0:
            raise RuntimeError("저장 결과의 페이지 수가 0입니다")
        # 모든 페이지 접근 가능 여부까지 검사
        for pno in range(check.page_count):
            _ = check[pno].rect
        check.close()
    except Exception as e:
        try:
            ...
            pass
        sys.exit(f"[오류] 출력 PDF 검증 실패. 기존 출력 파일은 교체하지 않았습니다: {e}")
