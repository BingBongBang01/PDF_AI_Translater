...
def extract_segments(doc: "pymupdf.Document", page_filter: set[int] | None,
    ...
    def make_seg(pno: int, bno: int, text: str, bbox: tuple,
                ...
                line_boxes: list[tuple] | None) -> Segment | None:
        text = text.strip()
        if not text:
            return None
        return Segment(
    ...
    for pno in range(doc.page_count):
        ...
        for block in data.get("blocks", []):
            ...
            line_infos = []  # 각 줄: (text, bbox, sizes, colors, bold_votes)
            ...
            if not table_like:
                ...
                for text, bbox, sizes, colors, bold_votes in line_infos:
...
def parse_args():
    ...
    ap.add_argument("--model-local", default=None,
                    help=f"로컬 모델 이름 (기본: {DEFAULT_MODELS['local']})")
