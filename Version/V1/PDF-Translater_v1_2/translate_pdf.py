...
def get_client():
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        sys.exit("[오류] 환경변수 ANTHROPIC_API_KEY가 없습니다.\n"
                 "       export ANTHROPIC_API_KEY=sk-ant-... 후 다시 실행하세요.")
    return anthropic.Anthropic()  # 429/5xx는 SDK가 기본 재시도
...
def translate_all_batches(client, args, system_prompt: str, template: str,
    ...
    for bi, batch in enumerate(batches, 1):
        ...
        for attempt in range(1, args.max_attempts + 1):
            ...
            try:
                raw = call_claude(client, args.model, system_prompt, prompt,
...
def parse_args():
    ...
    ap.add_argument("--model", default=DEFAULT_MODEL,
                    help="Anthropic 모델 ID (예: claude-sonnet-4-6, claude-haiku-4-5, claude-opus-4-8)")
