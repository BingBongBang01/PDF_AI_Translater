...
from pdf_engine.config.settings import (
    RUNTIME_REGISTRY,
    DEFAULT_LOCAL_RUNTIME, get_runtime_spec, SYSTEM_PROMPT_PATH,
    SYSTEM_PROMPT_LOCAL_PATH, USER_TEMPLATE_PATH, DEFAULT_MODELS, DEFAULT_LOCAL_MODEL_BY_DEVICE,
    DEFAULT_TERMINOLOGY_POLICY, _parse_local_devices, default_model_chain, MODEL_CHAIN_INFO,
    # GUI의 '중단' 버튼이 translate_pdf 모듈에서 이 이름들을 찾는다
    # (재노출하지 않으면 hasattr 검사에 걸려 중단 버튼이 아무 동작도 하지 않았다).
    STOP_EVENT, request_stop, reset_stop, stop_requested,
)
from pdf_engine.glossary.parser import GlossaryParser
from pdf_engine.pipeline import TranslationPipeline, PipelineState
from pdf_engine.preprocess.extractor import (
    extract_segments, merge_adjacent_segments,
    resolve_ocr_lang,
)
from pdf_engine.translator.providers_cloud import (
    get_key_pool,
)
from pdf_engine.translator.providers_local import (
    shutdown_local_runtime, make_local_entry,
)
from pdf_engine.translator.scheduler import mock_translate
from pdf_engine.postprocess.renderer import (
    rebuild_pdf,
)
from pdf_engine.postprocess.io_utils import (
    parse_pages, export_translations, import_translations,
)
from pdf_engine.postprocess.filenaming import (
    collapse_to_ranges, sidecar_path_for, write_progress_sidecar,
    load_resume_info, build_output_stem,
)
...
def parse_args():
    ...
    ap.add_argument("--provider", default=None,
                    choices=["anthropic", "gemini", "openai", "deepseek", "openrouter",
                             "groq", "ollama"],
    ...
    ap.add_argument("--model-gemini", default=None,
                    help=f"Gemini 키에 사용할 모델 (기본: {DEFAULT_MODELS['gemini']}). "
                         f"콤마로 여러 개를 적으면 그 순서대로 폴백한다 - Gemini는 한도가 "
                         f"모델별로 독립이라 키가 하나여도 앞 모델의 한도가 끝나면 다음 모델로 "
                         f"자동 전환된다. 예: --model-gemini "
                         f"\"{','.join(default_model_chain('gemini')[:3])},...\"")
    ...
    ap.add_argument("--api-timeout", type=float, default=180.0,
                    help="클라우드 API 요청 하나당 최대 대기 시간(초) (기본: 180). "
                         "특히 Gemini SDK는 이 값을 명시하지 않으면 서버가 응답을 영영 "
                         "안 줘도 요청이 무한정 걸린 채 예외조차 안 나서, 진행률이 0%%에서 "
                         "멈춘 것처럼 보이는 문제가 있었다. 배치가 아주 크면(사고형 모델 "
                         "포함) 늘려야 할 수 있다.")
    ...
    ap.add_argument("--glossary", default=None, help="용어집 파일 (.json, .yaml, .csv)")
    ap.add_argument("--glossary-profile", default="auto", help="용어집 프로필 (기본: auto - 문서 문맥 자동 분석)")
...
def main():
    ...
    if n_target == 0:
        ...
        glossary_map = GlossaryParser.load(args.glossary, args.glossary_profile)
        glossary_text = ""
        ...
        try:
            ...
            # 번역/후처리 단계에서 예상 못 한 오류가 나도 여기서 멈추지 않는다.
            # 여기서 예외를 그대로 올려보내면 아래 [4] 재구성 단계에 영영 도달하지 못해,
            # 이미 몇 시간 걸려 번역해 둔 결과가 PDF로 저장되지 않고 통째로 사라진다
            # (실제 확인된 문제: 로컬 NPU 번역이 99% 진행된 뒤 오류가 뜨며 전부 날아감).
            # 지금까지 번역된 것은 그대로 저장하고, 나머지는 원문 유지로 표시해
            # 출력 파일명의 미번역 구간으로 남긴다 -> 나중에 그 부분만 이어서 번역 가능.
            import traceback
            traceback.print_exc()
            print(f"[오류] 번역 단계에서 예외 발생 -> 여기까지 번역된 내용만 저장하고 "
                  f"나머지는 원문 유지로 진행합니다: {e}")
            segments = state.segments or segments
            aborted = True
        finally:
            shutdown_local_runtime()

        for s in segments:
            if s.needs_translation and s.translated is None:
                s.translated = s.raw_text if s.raw_text is not None else s.text
    ...
    if args.output:
        ...
        # 사용자가 디렉터리 없이 파일명만 입력한 경우 프로그램의 현재 작업 폴더가 아니라
        # 원본 PDF와 같은 폴더에 저장되도록 한다 (실제 확인된 문제: exe로 실행 시 cwd가
        # 프로그램 설치 폴더라서 출력이 원본과 다른 위치에 저장됨).
        if not out_path.is_absolute() and out_path.parent == Path("."):
            out_path = in_path.parent / out_path.name
        # 확장자를 안 붙였거나 다른 확장자를 붙인 경우 원본과 같은 확장자로 맞춘다
        # (실제 확인된 문제: 확장자가 사라져 탐색기에서 일반 파일로 보임).
        if out_path.suffix.lower() != in_path.suffix.lower():
            out_path = out_path.with_suffix(in_path.suffix)
...
def compress_pdf(src_path: Path) -> Path | None:
    """
    최종 산출물을 압축 최적화해 같은 파일명(src_path)으로 교체한다.
    ...
    건드리지 않는다.
    이전에는 압축본을 <stem>_compressed.pdf라는 별도 파일로 만들어 용량 큰 원본과
    압축본이 둘 다 남았다(실제 사용자 확인 문제). 이제는 임시 파일에 압축 저장한 뒤
    원본을 그 압축본으로 교체해, 최종적으로 파일 하나만(원래 이름 그대로) 남긴다.
    실패해도 원본엔 영향 없음(임시 파일만 지우고 None 반환, 원본 그대로 사용).
    """
    tmp_compressed = src_path.with_name(src_path.stem + ".compress_tmp" + src_path.suffix)
    try:
        ...
        try:
            ...
            pass
        cdoc.save(tmp_compressed, garbage=4, deflate=True, deflate_images=True,
                  deflate_fonts=True, clean=True)
        cdoc.close()
        before = src_path.stat().st_size
        after = tmp_compressed.stat().st_size
        os.replace(tmp_compressed, src_path)  # 압축본으로 원본을 교체 (별도 _compressed 파일 안 남김)
        pct = 100.0 * (1 - after / before) if before else 0.0
        print(f"[압축] {before / 1048576:.1f}MB -> {after / 1048576:.1f}MB ({pct:.0f}% 감소)")
        return src_path
    except Exception as e:
