    ...
    for seg in segments:
        ...
        if can_merge:
            ...
            if not conflict:
                ...
                # 문맥은 함께 번역하되 원본 블록 경계를 제어 토큰으로 보존한다.
                # 번역 후 이 토큰을 기준으로 다시 원래 bbox별로 배치한다.
                prev.text = prev.text.rstrip() + "\n|||SUB_SEPARATOR|||\n" + seg.text.lstrip()
                prev.line_boxes = (prev.line_boxes or [prev.bbox]) + (seg.line_boxes or [seg.bbox])
                prev.layout_boxes = (prev.layout_boxes or [prev.bbox]) + (seg.layout_boxes or [seg.bbox])
                prev.bbox = union
                absorbed.add(id(seg))
                n_merged += 1
                continue
...
def _insert_text_in_rect(page, rect, text, seg: Segment, font_scale: float) -> bool:
    ...
    if not text:
        return True
    page_rect = page.rect
    base_fs = max(5.5, seg.font_size * font_scale)
    min_fs = max(5.0, base_fs * 0.72)
    color = hex_to_rgb01(seg.color)
    weight = "bold" if seg.bold else "normal"
    original = pymupdf.Rect(rect)
    candidates = [original]
    # 병합 전 개별 블록은 서로 침범하지 않도록 확장하지 않는다.
    if not seg.layout_boxes or len(seg.layout_boxes) <= 1:
        max_extra = min(page_rect.height * 0.08, max(original.height * 1.5, base_fs * 3))
        expanded = pymupdf.Rect(original.x0, original.y0, original.x1, min(page_rect.y1 - 2, original.y1 + max_extra))
        if expanded.y1 > original.y1 + 1:
            candidates.append(expanded)
    if hasattr(page, "insert_htmlbox"):
        body = html_mod.escape(text).replace("\n", "<br>")
        css = ("* {margin:0;padding:0;font-family:sans-serif;"
               f"font-size:{base_fs:.2f}px;color:{seg.color};font-weight:{weight};"
               "line-height:1.18;}")
        for candidate in candidates:
            try:
                spare, scale = page.insert_htmlbox(candidate, f"<div>{body}</div>", css=css, scale_low=0.72)
    ...
    for candidate in candidates:
        size = base_fs
        while size >= min_fs:
            rc = page.insert_textbox(candidate, text, fontsize=size, fontname="korea", color=color, lineheight=1.18)
            if rc >= 0:
                return True
            size *= 0.95
    return False
...
def insert_translated_text(page, seg: Segment, font_scale: float) -> bool:
    ...
    boxes = seg.layout_boxes or [seg.bbox]
    if len(boxes) > 1:
        parts = re.split(r"\s*\|\|\|SUB_SEPARATOR\|\|\|\s*", text)
        if len(parts) == len(boxes):
            ok = True
            for box, part in zip(boxes, parts):
                ok = _insert_text_in_rect(page, box, part, seg, font_scale) and ok
            return ok
        print(f"  [경고] {seg.seg_id}: 병합 경계 토큰 수 불일치({len(parts)}/{len(boxes)}) -> 통합 영역 배치")
    return _insert_text_in_rect(page, seg.bbox, text.replace("|||SUB_SEPARATOR|||", "\n"), seg, font_scale)
