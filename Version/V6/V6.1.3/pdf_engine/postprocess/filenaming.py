...
def write_progress_sidecar(out_path: Path, base_stem: str,
    ...
    try:
        ...
        get_logger().log(f"[경고] 진행정보 사이드카(.progress.json) 저장 실패(무시하고 계속): {e}")
...
def load_resume_info(in_path: Path) -> dict | None:
    ...
    if sc.exists():
        try:
            ...
            get_logger().log(f"[경고] 사이드카 진행정보(.progress.json) 손상, 무시하고 파일명으로 시도: {e}")
    info = parse_resume_filename(in_path.stem)
    if info and info.get("unresolved"):
        get_logger().log("[경고] 파일명이 압축 형식(MULTIn)인데 사이드카(.progress.json)를 찾지 못해 "
