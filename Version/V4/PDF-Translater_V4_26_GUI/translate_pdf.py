...
def _ocr_script_plausibility(text: str, ocr_lang: str) -> tuple[bool, float, str]:
    ...
    if any(x in lang for x in ("jpn","jpn_vert")):
        matches=sum(jp(c) for c in compact); threshold=.42; min_matches=2
    elif "kor" in lang:
...
def _analyze_ocr_background(page, bbox) -> tuple[bool, tuple, float]:
    """
    OCR 영역이 단색 배경인지 검사한다.
    만화/사진처럼 색 변화가 큰 영역은 절대 사각형으로 덮지 않는다.
    반환: (덮기 안전 여부, 대표색, dominant_ratio)
    """
    try:
        rect=pymupdf.Rect(bbox)
        pix=page.get_pixmap(clip=rect, dpi=72, alpha=False)
        n=max(1,pix.n); raw=pix.samples
        colors=[]
        # 과도한 메모리 사용 방지 샘플링 + 32단계 양자화
        pixels=max(1,len(raw)//n); stride=max(1,pixels//2500)
        for px in range(0,pixels,stride):
            i=px*n
            if i+2>=len(raw): break
            rgb=(raw[i],raw[i+1],raw[i+2])
            colors.append(tuple((v//32)*32 for v in rgb))
        if len(colors)<8: return False,(1,1,1),0.0
        cnt=Counter(colors); dominant,count=cnt.most_common(1)[0]
        ratio=count/len(colors)
        # 단색/거의 단색 영역만 허용. 만화 그림은 보통 이 기준을 통과하지 못한다.
        safe=ratio>=0.72
        color=tuple(v/255 for v in dominant)
        return safe,color,ratio
    except Exception:
