    ...
    def next_alive_index(start: int) -> int | None:
        ...
        for e2 in pool:
            if not e2.alive and e2.revive_at is not None and now >= e2.revive_at:
                e2.alive = True
                e2.revive_at = None
                get_logger().log(f"  [부활] 키 {e2.label} 할당량 리셋 시간 경과 -> 재시도 대상으로 복귀")
            ...
            try:
                ...
                if is_rate_limit_error(e):
                    ...
                    if keys_tried_since_success % max(n_alive, 1) == 0:
                        ...
                        get_logger().log(f"  [batch {bi}/{len(batches)}] 살아있는 키 {n_alive}개 전부 "
                        ...
                        get_logger().log(f"  [batch {bi}/{len(batches)}] 키 {prev_label} 할당량 초과 "
