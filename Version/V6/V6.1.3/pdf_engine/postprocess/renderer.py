...
def _clear_rect_text(page, rect, fill_bg=(1, 1, 1)) -> None:
    """
    ...
    """
    try:
        page.add_redact_annot(pymupdf.Rect(rect), fill=fill_bg)
        apply_redactions_safe(page)
    except Exception:
        pass
    if fill_bg is not None:
        try:
            page.draw_rect(pymupdf.Rect(rect), color=None, fill=fill_bg, overlay=True)
        except Exception:
            pass
...
def _insert_text_in_rect(page, rect, text, seg: Segment, font_scale: float) -> bool:
    ...
    """
    text = _clean_pua_characters(_normalize_translation_text(text))
    if not text:
        return True
...
def _expand_rect_into_free_space(rect, obstacles, page_rect, pad: float = 1.5,
                                  max_grow: float = 60.0) -> "pymupdf.Rect":
    """
    번역문이 최소 폰트로도 원래 bbox에 안 들어갈 때, 주변의 다른 세그먼트(obstacles)나
    페이지 경계와 겹치지 않는 한도까지만 상하좌우로 넓혀서 여백을 확보한다.
    다른 텍스트의 위치는 건드리지 않고, 원래 그 사이에 있던 빈 공간만 사용한다.
    """
    r = pymupdf.Rect(rect)
    left_limit, right_limit = page_rect.x0, page_rect.x1
    top_limit, bottom_limit = page_rect.y0, page_rect.y1
    for ob in obstacles:
        ob = pymupdf.Rect(ob)
        if ob == r:
            continue
        # 세로 범위가 겹치는 이웃만 좌우 확장 한계에 반영
        if ob.y1 > r.y0 and ob.y0 < r.y1:
            if ob.x1 <= r.x0 and ob.x1 > left_limit:
                left_limit = ob.x1
            if ob.x0 >= r.x1 and ob.x0 < right_limit:
                right_limit = ob.x0
        # 가로 범위가 겹치는 이웃만 상하 확장 한계에 반영
        if ob.x1 > r.x0 and ob.x0 < r.x1:
            if ob.y1 <= r.y0 and ob.y1 > top_limit:
                top_limit = ob.y1
            if ob.y0 >= r.y1 and ob.y0 < bottom_limit:
                bottom_limit = ob.y0
    new_x0 = max(left_limit + pad, r.x0 - max_grow)
    new_x1 = min(right_limit - pad, r.x1 + max_grow)
    new_y0 = max(top_limit + pad, r.y0 - max_grow * 0.5)
    new_y1 = min(bottom_limit - pad, r.y1 + max_grow * 1.5)
    # 확장이 원래 bbox보다 작아지는 방향으로 튀는 경우 방지
...
def rebuild_pdf(doc, segments: list[Segment], font_scale: float) -> int:
    ...
    for pno in sorted(by_page):
        ...
        page_rect = page.rect
        obstacles = [pymupdf.Rect(s.bbox) for s in by_page[pno]]
        for idx, s in enumerate(by_page[pno]):
            if s.is_ocr and s.seg_id not in safe_ocr_ids:
                continue
            if insert_translated_text(page, s, font_scale):
                continue
            # 최소 폰트로도 안 들어감 - 겹치지 않는 주변 여백으로 확장해 재시도.
            others = obstacles[:idx] + obstacles[idx + 1:]
            expanded = _expand_rect_into_free_space(s.bbox, others, page_rect)
            text = (s.translated or "").replace("|||SUB_SEPARATOR|||", "\n").strip()
            if expanded != pymupdf.Rect(s.bbox) and _insert_text_in_rect(page, expanded, text, s, font_scale):
                continue
            truncated += 1
            get_logger().log(f"  [경고] {s.seg_id}: 번역문이 원래 영역보다 길어 최대 축소로도 넘칠 수 있음")
    return truncated