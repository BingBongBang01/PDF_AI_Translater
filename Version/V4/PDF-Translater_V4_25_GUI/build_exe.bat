...
echo [1/7] Locate Python executable (Looking for 3.12 or 3.14)
set "PYTHON_EXE="

:: 1순위: 안정성 및 호환성을 위해 Python 3.12를 먼저 찾습니다.
for /f "usebackq delims=" %%I in (`py -3.12 -c "import sys; print(sys.executable)" 2^>nul`) do set "PYTHON_EXE=%%I"

:: 2순위: 3.12가 없다면 Python 3.14를 찾습니다.
if not defined PYTHON_EXE (
    echo Python 3.12 not found. Trying Python 3.14...
    for /f "usebackq delims=" %%I in (`py -3.14 -c "import sys; print(sys.executable)" 2^>nul`) do set "PYTHON_EXE=%%I"
)

:: 3순위: 둘 다 없다면 기본 설정된 Python 3을 찾습니다.
if not defined PYTHON_EXE (
    echo Python 3.14 not found. Trying default Python 3...
    for /f "usebackq delims=" %%I in (`py -3 -c "import sys; print(sys.executable)" 2^>nul`) do set "PYTHON_EXE=%%I"
)

if not defined PYTHON_EXE (
    echo [ERROR] py launcher could not find Python 3.12, 3.14, or any Python 3.
