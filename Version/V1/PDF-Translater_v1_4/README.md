...
두 가지 방식 중 하나를 쓴다. 파일 방식이 우선순위가 높다(둘 다 있으면 파일을 씀).

```bash
# 방식 A) 키 파일 (권장 — 셸 히스토리에 키가 남지 않음)
echo "sk-ant-..." > api.txt                 # Anthropic, 또는
echo "AIza..."    > api.txt                 # Gemini
python translate_pdf.py doc.pdf --provider anthropic --api-key-file api.txt
python translate_pdf.py doc.pdf --provider gemini    --api-key-file api.txt

# api.txt를 직접 지정하지 않아도 현재 디렉터리에 api.txt가 있으면 자동으로 읽는다.
...
`api.txt`는 키 원문이 그대로 저장되는 평문 파일이다. 절대 git에 커밋하지 말 것 — 저장소를 쓴다면 `.gitignore`에 `api.txt`를 추가한다.

기본 provider는 `anthropic`(모델 `claude-sonnet-4-6`)이다. `--provider gemini` 지정 시 기본 모델은 `gemini-3.1-pro-preview`이며, `--model`로 다른 모델 ID를 직접 지정할 수 있다.

### 여러 계정 키 순환 사용 (할당량 초과 대비)

`api.txt`에 계정별 키를 한 줄에 하나씩 넣으면 자동으로 순환 사용한다. 빈 줄과 `#`으로 시작하는 줄은 무시한다.

```
# api.txt 예시 (Gemini 계정 3개)
AIza...계정1키
AIza...계정2키
AIza...계정3키
```

동작 방식: 현재 키로 계속 요청하다가 429(할당량 초과)가 뜨면 **대기 없이 즉시 다음 키로 전환**해서 이어간다. 모든 키를 한 바퀴 돌았는데도 전부 막혀 있으면 그때 서버가 알려준 대기시간만큼 자고 처음 키부터 다시 시도한다 — 계속 반복하므로 사람이 개입할 필요는 없다.

환경변수로 줄 때는 콤마로 구분한다: `export GEMINI_API_KEY=키1,키2,키3`

각 계정의 무료 등급 할당량으로 유료 등급 트래픽을 우회하는 셈이므로, 서비스 약관상 문제가 없는지는 직접 확인할 것.

Gemini 사용 시 주의: 번역 엔진 프롬프트(`prompts/`)는 애초에 Anthropic Messages API의 `system`/`user` 2단 구조에 맞춰 작성됐다. Gemini 호출부는 이를 `system_instruction`/`contents`로 그대로 매핑하고 `response_mime_type="application/json"`으로 JSON 출력을 강제하지만, 실제 API 키로 배치 1개 정도는 직접 실행해 출력 스키마가 어긋나지 않는지 확인할 것 — 이 환경에는 두 provider 모두 실제 키가 없어 mock 경로까지만 검증했다.
...
| `--provider` | anthropic | `anthropic` 또는 `gemini` |
| `--api-key-file` | ./api.txt 자동탐색 | 키 파일 경로. 한 줄에 키 하나, 여러 줄 = 여러 계정(순환 사용) |
| `--model` | provider별 기본값 | anthropic: `claude-sonnet-4-6`(대량은 `claude-haiku-4-5`, 고품질은 `claude-opus-4-8`) / gemini: `gemini-3.1-pro-preview` |
