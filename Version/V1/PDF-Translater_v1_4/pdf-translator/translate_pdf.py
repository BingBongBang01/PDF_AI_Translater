...
def load_api_keys(provider: str, explicit_path: str | None) -> list[str]:
    """
    API 키 로딩 (여러 개 지원 - 계정/키별로 한 줄씩).
    우선순위:
      1) --api-key-file로 지정한 파일 (한 줄에 키 하나, '#'으로 시작하는 줄은 주석 처리)
      2) 스크립트/현재 작업 디렉터리의 api.txt
      3) 환경변수 (ANTHROPIC_API_KEY 또는 GEMINI_API_KEY/GOOGLE_API_KEY, 콤마(,)로 여러 개 구분 가능)
    api.txt는 평문 저장이므로 .gitignore 등록 및 리포지토리 커밋 금지 권장.
    여러 계정 키를 한 번에 돌리는 것은 각 서비스의 이용약관(할당량 우회 관련)을 스스로 확인할 것.
    """
    ...
    for p in candidates:
        if p.is_file():
            ...
            if keys:
                return keys
            sys.exit(f"[오류] {p} 파일에 유효한 키가 없습니다.")
    ...
    sys.exit(
        ...
        f"       1) --api-key-file api.txt (계정별로 한 줄씩)\n"
        f"       2) 현재 디렉터리에 api.txt 파일 생성 (한 줄에 키 하나, 여러 줄 = 여러 계정)\n"
        f"       3) export {env_var}=키1,키2,키3"
    )


def get_clients(args):
    """args.provider에 맞는 (provider, [client, ...]) 반환. 키 개수만큼 클라이언트 생성."""
    provider = args.provider
    api_keys = load_api_keys(provider, args.api_key_file)
    print(f"[정보] API 키 {len(api_keys)}개 로드됨 "
          f"({'할당량 초과 시 순환 사용' if len(api_keys) > 1 else '단일 키'})")
    ...
    if provider == "anthropic":
        ...
        return provider, [anthropic.Anthropic(api_key=k) for k in api_keys]
    ...
    if provider == "gemini":
        ...
        return provider, [genai.Client(api_key=k) for k in api_keys]
...
def is_rate_limit_error(e: Exception) -> bool:
    """429/RESOURCE_EXHAUSTED(할당량 초과)인지 판별. 이 경우는 재시도 횟수를 소진시키지 않는다."""
    code = getattr(e, "code", None) or getattr(e, "status_code", None)
    if code == 429:
        return True
    s = str(e)
    return "429" in s or "RESOURCE_EXHAUSTED" in s or "rate_limit" in s.lower()
...
def translate_all_batches(provider: str, clients: list, args, system_prompt: str, template: str,
    ...
    last_call_time = 0.0
    n_keys = len(clients)
    key_idx = 0  # 배치 간에도 유지 (한 계정이 막히면 다음 실행부터는 그다음 계정을 계속 사용)
    ...
    for bi, batch in enumerate(batches, 1):
        ...
        while remaining:
            ...
            try:
                raw = call_llm(provider, clients[key_idx], args.model, system_prompt, prompt,
                ...
                if is_rate_limit_error(e):
                    ...
                    if args.max_rate_limit_retries and rl_retry > args.max_rate_limit_retries:
                        print(f"  [batch {bi}/{len(batches)}] 속도 제한 재시도 한도"
                    ...
                    if n_keys > 1:
                        ...
                        if keys_tried_since_success % n_keys == 0:
                            # 모든 키를 한 바퀴 돌았는데도 전부 할당량 초과 -> 대기 후 재시도
                            delay = extract_retry_delay(e)
                            print(f"  [batch {bi}/{len(batches)}] 키 {n_keys}개 전부 할당량 초과 - "
                                  f"{delay:.0f}초 대기 후 재시도")
                            time.sleep(delay)
                        else:
                            print(f"  [batch {bi}/{len(batches)}] 키 {prev_idx + 1}/{n_keys} 할당량 초과 "
                                  f"-> 키 {key_idx + 1}/{n_keys}로 전환")
                    else:
                        delay = extract_retry_delay(e)
                        print(f"  [batch {bi}/{len(batches)}] 할당량 초과(429) - {delay:.0f}초 "
                              f"대기 후 재시도 ({rl_retry}회째, 횟수 무제한)")
                        time.sleep(delay)
                    continue
                attempt += 1
                print(f"  [batch {bi}/{len(batches)}] 시도 {attempt}/{args.max_attempts} 실패: {e}")
