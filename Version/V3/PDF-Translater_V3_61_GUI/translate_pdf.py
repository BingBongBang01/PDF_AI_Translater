...
def insert_translated_text(page, seg: Segment, font_scale: float) -> bool:
    """
    번역문을 원본 블록 bbox 안에 자연스럽게 배치한다.
    원본의 줄 수를 번역문에 강제로 적용하지 않는다.
    폰트 크기를 과도하게 줄이지 않고, 필요하면 bbox의 아래쪽 여백만 제한적으로 확장한다.
    """
    original = pymupdf.Rect(seg.bbox)
    text = re.sub(r"[ \t]+", " ", (seg.translated or "").replace("\r", "")).strip()
    if not text:
        return True
    ...
    # 원본 영역 우선. 부족할 때만 아래쪽 빈 공간을 제한적으로 사용한다.
    # 가로 폭은 절대 변경하지 않아 목차 열/문단 정렬을 보존한다.
    candidates = [original]
    max_extra = min(page_rect.height * 0.08, max(original.height * 1.5, base_fs * 3))
    expanded = pymupdf.Rect(original.x0, original.y0, original.x1,
                            min(page_rect.y1 - 2, original.y1 + max_extra))
    ...
    if hasattr(page, "insert_htmlbox"):
        ...
        for rect in candidates:
            # 1차: 원본 크기에 가깝게 유지. 72% 아래로 자동 축소하지 않는다.
            css = ("* {margin:0;padding:0;font-family:sans-serif;"
                   f"font-size:{base_fs:.2f}px;color:{seg.color};font-weight:{weight};"
                   "line-height:1.18;}")
            try:
                spare, scale = page.insert_htmlbox(rect, f"<div>{body}</div>",
                                                   css=css, scale_low=0.72)
                if spare is None or spare >= 0:
                    return True
            except Exception:
                pass

    # fallback: 원본 크기에서 조금씩만 축소
    for rect in candidates:
        size = base_fs
        while size >= min_fs:
            rc = page.insert_textbox(rect, text, fontsize=size,
                                     fontname="korea", color=color, lineheight=1.18)
            if rc >= 0:
                return True
            size *= 0.95

    # 실패 시 4pt까지 억지로 축소하지 않고 실패로 반환하여 원문 유지 판정 가능
    return False
