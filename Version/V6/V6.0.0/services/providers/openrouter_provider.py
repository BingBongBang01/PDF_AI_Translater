...
class OpenRouterProvider(BaseProvider):
    ...
    def list_models(self) -> dict[str, ModelCapability]:
        return {"openrouter/auto": ModelCapability(context_window=100000, supports_streaming=True)}
    def translate(self, request: TranslationRequest) -> TranslationResponse:
        return TranslationResponse(request.request_id, request.chunk.chunk_id, f"[OpenRouter: {request.chunk.text}]", "stop", {})
    def stream_translate(self, request: TranslationRequest) -> Generator[str, None, TranslationResponse]:
        yield "[OpenRouter Stream] "
        return TranslationResponse(request.request_id, request.chunk.chunk_id, f"[OpenRouter Stream: {request.chunk.text}]", "stop", {})
