...
class Segment:
    ...
    line_boxes: list[tuple] | None = None  # 원본 각 줄 bbox: 번역문 재배치 시 원본 문단 폭/높이 보존
    ...
    for pno in range(doc.page_count):
        ...
        for block in data.get("blocks", []):
            if block.get("type") != 0:           # 0 = 텍스트 블록
                continue
            lines, sizes, colors, bold_votes, line_boxes = [], [], [], [], []
            for line in block.get("lines", []):
                line_text = "".join(sp.get("text", "") for sp in line.get("spans", []))
                if line_text.strip():
                    lines.append(line_text)
                    if line.get("bbox"):
                        line_boxes.append(tuple(line["bbox"]))
...
def merge_adjacent_segments(segments: list[Segment]) -> list[Segment]:
    ...
    for seg in segments:
        ...
        if can_merge:
            ...
            if not conflict:
                # 번역 문맥은 하나로 합치되 원본의 블록/줄 경계는 보존한다.
                # 재구성 단계에서 이 줄 정보를 사용해 원본 문단이 차지한 폭과 높이에 맞춰 배치한다.
                prev.text = prev.text.rstrip() + "\n" + seg.text.lstrip()
                prev.line_boxes = (prev.line_boxes or [prev.bbox]) + (seg.line_boxes or [seg.bbox])
                prev.bbox = union
                absorbed.add(id(seg))
                n_merged += 1
                continue
...
def _layout_text_to_original_lines(text: str, seg: Segment) -> str:
    """
    번역문을 원본 문단의 줄 폭 비율에 맞춰 재배치한다.
    번역 자체에는 원본의 강제 줄바꿈을 요구하지 않고, PDF 삽입 직전에만 레이아웃을 맞춘다.
    이렇게 하면 문맥 번역 품질은 유지하면서 목차/문단의 원래 폭과 세로 사용 영역을 최대한 보존한다.
    """
    text = re.sub(r"[ \t]+", " ", (text or "").replace("\r", "")).strip()
    if not text:
        return text
    boxes = [b for b in (seg.line_boxes or []) if len(b) == 4 and b[2] > b[0]]
    if len(boxes) <= 1:
        return text

    # 원본 줄 폭을 문자 수용량의 가중치로 사용한다.
    widths = [max(1.0, b[2] - b[0]) for b in boxes]
    total_w = sum(widths)
    # 한국어는 영어보다 같은 폭에 더 적은 글자가 들어가는 경향이 있으므로
    # 실제 번역문 길이를 기준으로 각 원본 줄에 비례 배분한다.
    target_counts = [max(1, round(len(text) * w / total_w)) for w in widths]

    # 단어 경계를 우선하여 줄을 나눈다. 너무 긴 토큰만 문자 단위로 분할한다.
    words = text.split(" ")
    lines=[]; pos=0
    for i, cap in enumerate(target_counts[:-1]):
        if pos >= len(words): break
        cur=[]; n=0
        while pos < len(words):
            word=words[pos]
            add=len(word) + (1 if cur else 0)
            if cur and n + add > cap:
                break
            cur.append(word); n += add; pos += 1
            if n >= cap: break
        if not cur and pos < len(words):
            cur=[words[pos]]; pos += 1
        lines.append(" ".join(cur))
    if pos < len(words):
        lines.append(" ".join(words[pos:]))
    return "\n".join(x for x in lines if x)


def insert_translated_text(page, seg: Segment, font_scale: float) -> bool:
    """원본 문단 영역과 줄 구조를 기준으로 번역문을 삽입한다."""
    rect = pymupdf.Rect(seg.bbox)
    fs = max(4.0, seg.font_size * font_scale)
    laid_out = _layout_text_to_original_lines(seg.translated or "", seg)

    # 원본 줄 간격을 CSS line-height로 환산한다.
    boxes = seg.line_boxes or []
    if len(boxes) >= 2:
        centers=[(b[1]+b[3])/2 for b in boxes]
        gaps=[centers[i+1]-centers[i] for i in range(len(centers)-1) if centers[i+1] > centers[i]]
        line_height = (sum(gaps)/len(gaps))/max(fs, 1.0) if gaps else 1.20
        line_height = min(1.55, max(1.0, line_height))
    else:
        line_height = 1.20

    if hasattr(page, "insert_htmlbox"):
        body = html_mod.escape(laid_out).replace("\n", "<br>")
        ...
        css = ("* {margin:0;padding:0;font-family:sans-serif;"
               ...
               f"line-height:{line_height:.3f};}}")
        try:
            # 원본 bbox 안에서만 자동 축소. bbox 자체를 늘리지 않아 주변 요소 침범을 막는다.
            spare, _scale = page.insert_htmlbox(rect, f"<div>{body}</div>",
                                                css=css, scale_low=0.35)
    ...
    size = fs
    while size >= 4.0:
        rc = page.insert_textbox(rect, laid_out, fontsize=size,
                                 fontname="korea", color=color,
                                 lineheight=line_height)
        if rc >= 0:
            return True
        size *= 0.92
    page.insert_textbox(rect, laid_out, fontsize=4.0,
                        fontname="korea", color=color, lineheight=line_height)
    return False
...
RESUME_FILENAME_RE = re.compile(
    r"^(?P<base>.+?)_(?:translated|T)_(?P<tranges>\d{3}-\d{3}(?:_\d{3}-\d{3})*)"
...
def parse_resume_filename(stem: str) -> dict | None:
    ...
    if not m:
        return None
    t_ranges = []
    for part in m.group("tranges").split("_"):
        a, b = part.split("-")
        t_ranges.append((int(a), int(b)))
    ...
    if u_ranges == [(0, 0)]:  # 완역 sentinel
        u_ranges = []
    return {"base": m.group("base"), "t_ranges": t_ranges, "t_start": t_ranges[0][0], "t_end": t_ranges[-1][1], "u_ranges": u_ranges}


def build_output_stem(base_stem: str, t_ranges: list[tuple[int, int]],
                      u_ranges: list[tuple[int, int]]) -> str:
    """번역 완료/미완료 페이지 집합을 각각 연속 구간으로 파일명에 기록한다."""
    fmt = lambda n: f"{n:03d}"
    t_display = t_ranges if t_ranges else [(0, 0)]
    u_display = u_ranges if u_ranges else [(0, 0)]
    t_part = "_translated_" + "_".join(f"{fmt(a)}-{fmt(b)}" for a, b in t_display)
    u_part = "_untranslated_" + "_".join(f"{fmt(a)}-{fmt(b)}" for a, b in u_display)
...
def main():
    ...
    final_untranslated_pages = all_pages - final_translated_pages
    ...
    if args.output:
        ...
        out_stem = build_output_stem(base_stem, final_translated_ranges,
