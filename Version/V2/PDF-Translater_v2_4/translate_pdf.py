...
def _try_start_one(exe: str, port: int, args) -> bool:
    ...
    # lemonade-server-dev 내부 print()의 유니코드 문자(예: '•')에서 UnicodeEncodeError로 즉시 죽는다.
...
def load_local_model(args) -> bool:
    """
    /api/v1/load로 모델을 명시적 ctx_size와 함께 로드한다.
    번역 엔진 프롬프트가 길기 때문에 기본 컨텍스트(보통 4096 이하)로는
    시스템 프롬프트 뒤쪽(JSON 출력 지시)이 잘려나가 모델이 JSON이 아닌 일반 텍스트로
    응답하는 문제가 생길 수 있다. ctx_size를 넉넉히 키워서 이를 방지한다.
    """
    import urllib.request
    model = args.model_local or DEFAULT_MODELS["local"]
    payload = json.dumps({"model": model, "ctx_size": args.local_ctx_size}).encode("utf-8")
    ...
    try:
        with urllib.request.urlopen(req, timeout=120) as r:
            ok = r.status == 200
            if ok:
                print(f"[로컬] 모델 로드 완료: {model} (ctx_size={args.local_ctx_size})")
            else:
                print(f"[로컬][경고] 모델 로드 응답 코드 {r.status} (기존 로드 설정으로 계속 진행)")
            return ok
    except Exception as e:
        print(f"[로컬][경고] 명시적 로드(ctx_size 지정) 실패, 기존 로드 상태로 계속 진행: {e}")
        return False
        ...
        while remaining:
            ...
            try:
                raw = call_llm(entry.provider, entry.client, entry.model,
                              system_prompt_local if entry.is_local else system_prompt, prompt,
