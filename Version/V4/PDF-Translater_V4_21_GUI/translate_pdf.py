...
def extract_segments(doc: "pymupdf.Document", page_filter: set[int] | None,
    ...
    for pno in range(doc.page_count):
        ...
        if total_chars < 20:
            ...
            if has_images:
                try:
                    ocr_textpage = page.get_textpage_ocr(flags=0, full=True, dpi=200)
                    ...
                    if not ocr_warned["done"]:
                        ...
                        print("       해결: Tesseract OCR을 설치하세요 "
                              "(https://github.com/tesseract-ocr/tesseract) - "
                              "설치 후 이 스크립트가 자동으로 인식합니다. "
                              "설치 안 하면 이 페이지는 원문(빈 상태)으로 남습니다.")
        ...
        for block in data.get("blocks", []):
            ...
            block_vertical = sum(1 for li in line_infos if li[5]) > len(line_infos) / 2

            if block_vertical:
                # 세로쓰기(일본어 종서 등)에서는 한 문단이 여러 개의 세로줄(컬럼, 서로 다른
                # x0)로 구성되는 게 정상이다(오른쪽에서 왼쪽으로 읽는 여러 컬럼). 이걸 x0
                # 분산으로 표 판정하면 정상 문단을 컬럼별로 잘게 쪼개버려서(예: "IPv4 주소는
                # 통신 상대를 식별합니다"가 "IPv4"/"주소는"/"통신"/... 처럼 낱글자 수준으로
                # 쪼개짐), 그 결과 각 조각이 원래의 좁은 컬럼 bbox에 억지로 맞춰지며 극단적으로
                # 축소돼 읽기 힘들어진다("1줄이 2줄로 들어가며 글자가 작아짐" 현상의 원인).
                #
                # 세로쓰기는 y0(세로 시작 위치) 절대편차로 판정하면 안 된다 - 글자 수가 적은
                # 짧은 컬럼은 y0(상단)가 안쪽으로 들어와도 y1(하단/기준선)은 다른 컬럼과 같은
                # 경우가 흔해서(같은 기준선에서 위로 뻗어나가는 구조), y0만 보면 편차가 커
                # 보여도 실제로는 같은 문단이다. 대신 각 줄의 [y0,y1] 구간이 서로 얼마나
                # 겹치는지(y-overlap)로 판정한다 - 표의 다른 행이면 겹침이 거의 없고, 같은
                # 문단의 여러 컬럼이면 대부분 겹친다.
...
def _insert_text_in_rect(page, rect, text, seg: Segment, font_scale: float) -> bool:
    ...
    if hasattr(page, "insert_htmlbox"):
        ...
        # 지원 확인됨 - 실제 렌더링 테스트에서 무시되고 가로로 나오는 것을 확인했다).
        # 대신 insert_htmlbox가 공식 지원하는 rotate 파라미터(0/90/180/270)로 텍스트 블록
        # 전체를 90도 회전시켜 세로로 긴 영역에 맞춰 넣는다 - 진짜 종서(글자가 위→아래로
        # 쌓이는 형태)는 아니지만, 가로로 삐져나오는 것보다 원본 레이아웃의 방향을 존중한다.
...
def rebuild_pdf(doc, segments: list[Segment], font_scale: float) -> int:
    ...
    for pno in sorted(by_page):
        page = doc[pno]
        # OCR로 얻은 세그먼트는 원본 PDF에 텍스트 레이어 자체가 없으므로(이미지 위에 아무
        # 텍스트도 없음) 지울 게 없다 - redact 자체를 생략해 불필요한 처리와 부작용을 피한다.
