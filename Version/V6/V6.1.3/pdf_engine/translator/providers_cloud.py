...
from pdf_engine.validator.api_validator import is_rate_limit_error, is_auth_error, is_quota_exhaustion, is_permanent_exhaustion
...
def get_key_pool(args) -> list[KeyEntry]:
    ...
    if pool:
        ...
        get_logger().log(f"[정보] API 키 {len(pool)}개 로드됨 -> {summary} "
