...
echo ==========================================
echo PDF Translater V3.82 Windows EXE Builder (onefile)
echo ==========================================
echo.
...
if exist "dist\PDF-Translater-v3.82.exe" del /q "dist\PDF-Translater-v3.82.exe"
if exist "PDF-Translater-v3.82.spec" del /q "PDF-Translater-v3.82.spec"

echo.
echo [5/6] PyInstaller 빌드 (--onefile: 결과물이 exe 파일 하나)
REM --windowed(=--noconsole): 실행 시 콘솔 창 없이 GUI만 뜬다.
REM --onefile: dist 폴더 안에 exe 딱 하나만 생성 (배포/실행이 파일 하나로 끝남).
"%PYTHON_EXE%" -m PyInstaller --noconfirm --clean --onefile --windowed ^
 --name PDF-Translater-v3.82 ^
 --collect-all pymupdf ^
 --collect-all pypdf ^
...
echo.
echo [6/6] 빌드 결과 검사
if not exist "dist\PDF-Translater-v3.82.exe" (
...
echo.
echo ==========================================
echo 빌드 완료 - exe 파일 하나로 실행됩니다 (콘솔 창 없음)
echo dist\PDF-Translater-v3.82.exe
echo.
echo 주의: 이 exe만 다른 폴더/컴퓨터로 옮겨도 그대로 실행됩니다.
echo       api.txt/config.json 등 사용자 데이터는 exe와 별개로 관리되며
echo       API 키는 %%APPDATA%%\PDFTranslaterGUI\config.json 에 저장됩니다.
echo ==========================================
pause
exit /b 0
