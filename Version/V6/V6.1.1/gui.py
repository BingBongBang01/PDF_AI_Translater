...
def _real_python_candidates():
    """
    PC에 설치된 실제 Python 인터프리터 후보를 전부 모은다 (버전 무관, 스토어 alias 제외).
    특정 버전(예전엔 3.12)에 고정하지 않는 이유: 이 앱의 의존성(PyMuPDF, customtkinter 등)은
    최신 Python에서도 대부분 정상 동작하고, 사용자 PC에 이미 설치된 Python을 그대로 쓸 수 있으면
    별도 버전을 새로 설치하게 만들 필요가 없다. 'py -0p'로 설치된 모든 버전을 나열하고,
    최신 버전이 보통 먼저 나오므로 그 순서를 그대로 우선순위로 쓴다.
    """
    found=[]
    if os.name=="nt":
        try:
            ...
            pass
        # py 런처 자체가 없는 환경 대비 폴백: 흔히 쓰이는 설치 경로를 직접 뒤진다.
        progs=Path(os.environ.get("LOCALAPPDATA",""))/"Programs"/"Python"
        if progs.is_dir():
            for exe in sorted(progs.glob("Python3*/python.exe"), reverse=True):
...
def _pick_working_python():
    """필요한 패키지(PyMuPDF)가 이미 설치된 Python을 우선으로 고른다 (버전 무관).
    없으면 현재 실행 중인 인터프리터, 그것도 스토어 alias면 후보 중 첫번째를 쓴다."""
...
def _double_click_bootstrap():
    """
    .py 연결이 WindowsApps Python으로 잡혀 있어도 실제 Python(버전 무관)으로 재실행.
...
class App(ctk.CTk):
    ...
    def build_ai_tab(self, tab):
        ...
        adv = ctk.CTkFrame(n, fg_color="transparent"); adv.pack(fill="x")
        self._adv_visible = tk.BooleanVar(value=False)
        self.adv_toggle_btn = ctk.CTkButton(adv, text="▸ 고급 설정 (배치 크기 / max_tokens)", fg_color="transparent",
                                            text_color=PRIMARY, hover=False, anchor="w",
                                            command=self.toggle_advanced)
        self.adv_toggle_btn.pack(anchor="w", pady=(8,0))
        self.adv_frame = ctk.CTkFrame(n, fg_color="transparent")
        for i,(label,var) in enumerate((("배치 문자 수",self.chars),("세그먼트 수",self.segs),("max_tokens",self.tokens))):
            ctk.CTkLabel(self.adv_frame, text=label).grid(row=0, column=i*2, padx=(0 if i==0 else 16,6), pady=8)
            ctk.CTkEntry(self.adv_frame, textvariable=var, width=110).grid(row=0, column=i*2+1)
        # 기본 접힘 상태 (adv_frame은 pack하지 않음)
