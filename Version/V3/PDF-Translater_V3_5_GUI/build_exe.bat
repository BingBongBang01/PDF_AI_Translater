...
if exist "dist\PDF-Translater-v3.5" rmdir /s /q "dist\PDF-Translater-v3.5"

echo.
echo [5/6] PyInstaller 빌드
"%PYTHON_EXE%" -m PyInstaller --noconfirm --clean --onedir --windowed ^
 --name PDF-Translater-v3.5 ^
 --collect-all pymupdf ^
 --collect-all pypdf ^
...
echo.
echo [6/6] 빌드 결과 검사
if not exist "dist\PDF-Translater-v3.5\PDF-Translater-v3.5.exe" (
...
echo ==========================================
echo 빌드 완료
echo dist\PDF-Translater-v3.5\PDF-Translater-v3.5.exe
echo ==========================================
pause
exit /b 0
