...
from pdf_engine.utils.text import _clean_pua_characters
...
from pdf_engine.config.settings import LETTER_RE
...
def _build_fake_blocks_from_bubbles(page, regions: list[tuple], ocr_lang: str,
    ...
    """
    from pdf_engine.preprocess import ocr as manga_ocr_engine
...
def extract_segments(doc: "pymupdf.Document", page_filter: set[int] | None,
    ...
    def make_seg(pno: int, bno: int, text: str, bbox: tuple,
        ...
        text = _clean_pua_characters(text.strip())
        if not text:
            return None
        if is_ocr:
    ...
    for pno in range(doc.page_count):
        ...
        if total_chars < 20 or existing_text_unreliable:
            ...
            if has_images and ocr_lang is None:
                ...
                try:
                    ...
                    if not ocr_warned["done"]:
                        get_logger().log(f"[경고] {pno + 1}페이지가 스캔본으로 보이지만 OCR을 사용할 수 없습니다: {e}")
                        if resolved_tessdata:
                            get_logger().log(f"       tessdata 경로는 찾음({resolved_tessdata})인데도 실패했습니다. "
                                  f"Tesseract 실행파일 자체나 언어 데이터가 손상됐을 수 있습니다.")
                        else:
                            get_logger().log("       Tesseract의 언어 데이터(tessdata) 폴더를 자동으로 찾지 못했습니다. "
                        ...
                        get_logger().log("       (OCR을 못 쓰면 이 페이지는 원문 없이 빈 상태로 남습니다.)")
                        ocr_warned["done"] = True
                        ocr_warned["done"] = True
        ...
        for block in data.get("blocks", []):
            ...
            for line in block.get("lines", []):
                line_text = _clean_pua_characters("".join(sp.get("text", "") for sp in line.get("spans", [])))
            ...
            if not table_like:
                ...
                # 일반 문단: 기본적으로 블록 전체를 한 세그먼트로 묶어 문맥/번역 품질을
                # 지킨다. 다만 pymupdf는 제목(큰 폰트)과 바로 이어지는 본문(작은 폰트)을
                # 서로 다른 문단인데도 같은 "블록"으로 묶어버리는 경우가 실제로 있다
                # (실제 확인된 문제: "Sample PDF"(36pt 제목)과 "This is a simple PDF
                # file..."(17pt 본문)가 한 블록으로 묶임). 이걸 하나의 세그먼트로 만들면
                # make_seg가 폰트 크기를 최빈값 하나로만 정하므로, 본문이 제목 크기로
                # (또는 그 반대로) 렌더링되어 심하게 깨진다. 그래서 줄 사이 폰트 크기가
                # 큰 폭으로(1.4배 초과) 급격히 바뀌는 지점에서는 블록을 여러 세그먼트로
                # 쪼갠다 - 크기가 비슷한 일반 다행 문단은 전혀 영향받지 않는다.
                size_groups: list[list[tuple]] = []
                cur_group: list[tuple] = []
                cur_size = None
                for li in line_infos:
                    li_size = li[2][0] if li[2] else 11.0
                    if cur_group and cur_size and li_size > 0 and cur_size > 0 and \
                       max(li_size, cur_size) / min(li_size, cur_size) > 1.4:
                        size_groups.append(cur_group)
                        cur_group = []
                    cur_group.append(li)
                    cur_size = li_size
                if cur_group:
                ...
                for group in size_groups:
                    g_text = "\n".join(li[0] for li in group)
                    gx0 = min(li[1][0] for li in group)
                    gy0 = min(li[1][1] for li in group)
                    gx1 = max(li[1][2] for li in group)
                    gy1 = max(li[1][3] for li in group)
                    g_sizes = [s for li in group for s in li[2]]
                    g_colors = [c for li in group for c in li[3]]
                    g_bold = [b for li in group for b in li[4]]
                    g_line_boxes = [li[1] for li in group]
                    seg = make_seg(pno, bno, g_text, (gx0, gy0, gx1, gy1), g_sizes, g_colors,
                                  g_bold, g_line_boxes, vertical=block_vertical, is_ocr=is_ocr_page)
                    if seg:
                        segments.append(seg)
                        if is_ocr_page:
                ...
                # 표/다단 의심 블록 내에서도, 연속된 줄들의 시작 x좌표(x0)가 비슷하면(25pt 이내)
                # 동일한 문단의 여러 줄로 묶는다. 이렇게 하면 화자 라벨(예: JAMES: X=200)과
                # 대화 본문(X=320)이 블록 내에 함께 있더라도, 대화 본문 줄들(X=320)은 통문단 1개로
                # 깔끔하게 묶여 번역문맥이 유지되고 폰트 크기가 균일하게 유지된다.
                line_groups: list[list[tuple]] = []
                curr_group: list[tuple] = []
                for li in line_infos:
                    if not curr_group:
                        curr_group.append(li)
                    else:
                        prev = curr_group[-1]
                        x0_diff = abs(li[1][0] - prev[1][0])
                        prev_fs = prev[2][0] if prev[2] else 11.0
                        y_gap = max(0.0, li[1][1] - prev[1][3])
                        if x0_diff <= 25.0 and y_gap <= prev_fs * 2.5:
                            curr_group.append(li)
                        else:
                ...
                for group in line_groups:
                    ...
                    gx1 = max(li[1][2] for li in group)
                    gy1 = max(li[1][3] for li in group)
                    g_bbox = (gx0, gy0, gx1, gy1)
                    g_sizes = [s for li in group for s in li[2]]
                    g_colors = [c for li in group for c in li[3]]
                    g_bold = [b for li in group for b in li[4]]
                    g_line_boxes = [li[1] for li in group]
                    g_vert = group[0][5]
                    seg = make_seg(pno, bno, g_text, g_bbox, g_sizes, g_colors, g_bold, g_line_boxes,
                                  vertical=g_vert, is_ocr=is_ocr_page)
                    if seg:
                        segments.append(seg)
                        if is_ocr_page:
                            ocr_seg_indices_by_block.setdefault(block_idx, []).append(len(segments) - 1)
