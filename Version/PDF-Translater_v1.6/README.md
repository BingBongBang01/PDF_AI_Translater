...
출력 파일명에는 **항상** `_translated_###-@@@_untranslated_$$$-%%%`가 붙는다(둘 다 항상 존재).

- `_translated_###-@@@`: 이 문서가 다루는 전체 페이지 스코프(예: 1~100페이지)
- `_untranslated_$$$-%%%`: 원인 불문(할당량 소진, API 오류, JSON 파싱 실패 등)하고 결국 원문 그대로 남은 페이지. **산발적으로 여러 군데 흩어져도 각각 별도 구간으로 이어붙는다**
- 완역되어 미번역 구간이 하나도 없으면 `000-000`으로 표시(다음에 이어서-번역에 넣었을 때 건너뛰기용 sentinel)
- 파일명이 너무 길면 자동으로 축약: `_translated`→`_T`, `_untranslated`→`_unT`

예: 100페이지 문서에서 10~20페이지와 30~34페이지가 (서로 다른 이유로) 실패했다면:
```
doc_translated_001-100_untranslated_010-020_030-034.pdf
```

**이 출력 파일을 그대로 다시 입력으로 넣으면** 파일명에서 미번역 구간을 전부 읽어 `--pages 10-20,30-34`가 자동으로 설정되고, 그 페이지들만 다시 번역을 시도한다. 이번 실행에서도 일부가 또 실패하면 새로운 미번역 구간으로 갱신되고(예: 30-34는 성공, 10-20 중 15만 재실패 → `_untranslated_015-015`), 전부 성공하면 `_untranslated_000-000`이 되어 더 이상 할 게 없다는 뜻이 된다. `000-000`인 파일을 다시 넣으면 즉시 "할 일 없음"으로 종료한다.

```bash
# 1차 실행 -> 일부 페이지 실패 -> doc_translated_001-100_untranslated_010-020_030-034.pdf 생성됨
python translate_pdf.py doc.pdf

# 그대로 재실행만 하면 자동으로 010-020, 030-034만 이어서 번역
python translate_pdf.py doc_translated_001-100_untranslated_010-020_030-034.pdf
```

`_translated_###-@@@`(전체 스코프)는 최초 실행 때의 범위로 고정되며, 재번역 대상만 좁혀서 처리해도 바뀌지 않는다. 이미 성공한 페이지는 다시 추출/재번역하지 않는다(재개 시 `--pages`가 실패 구간으로만 자동 설정되므로). `-o`로 출력 파일명을 직접 지정하면 이 자동 이름 생성/재개 감지는 건너뛴다.
