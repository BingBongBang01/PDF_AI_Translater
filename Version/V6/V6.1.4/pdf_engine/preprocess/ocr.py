...
def get_engine():
    ...
    try:
        ...
        get_logger().log("[manga-ocr] 모델 로드 중(최초 1회, 몇 초~수십 초 소요될 수 있음)...")
        _engine = MangaOcr()
        get_logger().log("[manga-ocr] 로드 완료")
        return _engine
    except Exception as e:
        get_logger().log(f"[manga-ocr][경고] 로드 실패, 이후 Tesseract만 사용합니다: {e}")
        _load_failed = True
        return None
...
def recognize(pil_image) -> str | None:
    ...
    try:
        ...
        get_logger().log(f"[manga-ocr][경고] 인식 실패: {e}")
        return None
