...
class ClaudeProvider(BaseProvider):
    ...
    def list_models(self) -> dict[str, ModelCapability]:
        return {"claude-3-opus": ModelCapability(context_window=200000, supports_streaming=True)}
    def translate(self, request: TranslationRequest) -> TranslationResponse:
        return TranslationResponse(request.request_id, request.chunk.chunk_id, f"[Claude: {request.chunk.text}]", "stop", {})
    def stream_translate(self, request: TranslationRequest) -> Generator[str, None, TranslationResponse]:
        yield "[Claude Stream] "
        return TranslationResponse(request.request_id, request.chunk.chunk_id, f"[Claude Stream: {request.chunk.text}]", "stop", {})
