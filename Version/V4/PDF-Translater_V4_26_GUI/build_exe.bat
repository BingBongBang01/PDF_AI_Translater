...
echo [1/7] Locate installed Python
set "PYTHON_EXE="
for /f "delims=" %%I in ('py -c "import sys;print(sys.executable)" 2^>nul') do set "PYTHON_EXE=%%I"
if not defined PYTHON_EXE for /f "delims=" %%I in ('python -c "import sys;print(sys.executable)" 2^>nul') do set "PYTHON_EXE=%%I"
if not defined PYTHON_EXE (
 echo [ERROR] Installed Python not found.
 pause
 exit /b 1
)
echo Using Python: %PYTHON_EXE%
"%PYTHON_EXE%" -c "import sys; assert sys.version_info >= (3,10)" || (pause & exit /b 1)
