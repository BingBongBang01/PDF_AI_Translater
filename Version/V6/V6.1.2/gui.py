...
PROVIDER_MODELS = {
    "gemini": {
        ...
        "models": ["gemini-2.5-flash", "gemini-2.5-pro", "gemini-2.0-flash", "gemini-1.5-pro", "gemini-1.5-flash", "직접 입력"]
    },
    "anthropic": {
    ...
    "openrouter": {
        "default": "auto",
        "models": ["auto", "anthropic/claude-3.7-sonnet", "google/gemini-2.5-flash", "openai/gpt-4o-mini", "deepseek/deepseek-chat", "직접 입력"]
    },
    "groq": {
...
CLOUD_PRESETS = {
    "gemini-2.5-flash": (3500, 25, 8192),
    "gemini-2.0-flash": (3500, 25, 8192),
    "gemini-1.5-flash": (3000, 20, 8192),
    "gpt-4o-mini": (3500, 25, 8192),
    "claude-3-5-haiku-20241022": (3000, 20, 8192),
    "gemini-2.5-pro": (2800, 20, 8192),
    "gemini-1.5-pro": (2800, 20, 8192),
    "claude-3-7-sonnet-20250219": (2800, 20, 8192),
    "claude-3-5-sonnet-20241022": (2800, 20, 8192),
    "claude-3-opus-20240229": (2500, 16, 4096),
    "gpt-4o": (2800, 20, 8192),
    "gpt-4.5-preview": (2500, 18, 8192),
    "deepseek-chat": (2800, 20, 8192),
    "llama-3.3-70b-versatile": (2800, 20, 8192),
    "deepseek-reasoner": (2000, 15, 12288),
    "o3-mini": (2000, 15, 12288),
    "o1": (2000, 15, 12288),
}
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.cloud_chars = tk.StringVar(value="2800")
        self.cloud_segs = tk.StringVar(value="20")
        self.cloud_tokens = tk.StringVar(value="8192")
    ...
    def add_api(self, provider="gemini", initial_model=None):
        ...
        def_m = initial_model or get_default_model(provider)
        mv = tk.StringVar(value=def_m)
        key = tk.StringVar()
        ...
        provider_combo = ctk.CTkComboBox(
            ...
            values=presets,
            width=110,
            command=lambda choice: self._on_provider_select(choice, pv, provider_combo, mv, model_combo)
        )
        ...
        avail_models = get_provider_models(provider)
        if def_m and def_m not in avail_models:
            avail_models = [def_m] + list(avail_models)
        ...
        model_combo = ctk.CTkComboBox(
            ...
            values=avail_models,
            width=200,
            command=lambda choice: self._on_model_select(choice, mv, model_combo)
        )
        model_combo.set(def_m)
        model_combo.pack(side="left", padx=(0, 4))

        item = [row, on, pv, key, mv, model_combo]
    ...
    def _on_provider_select(self, choice, pv_var, provider_combo, mv_var, model_combo):
        if choice == "직접 입력":
            pv_var.set("")
            provider_combo.set("")
            provider_combo.focus_set()
            models = ["default", "직접 입력"]
            def_m = "default"
        else:
            models = get_provider_models(choice)
            def_m = get_default_model(choice)
    ...
    def _on_model_select(self, choice, mv_var, model_combo):
        if choice == "직접 입력":
            mv_var.set("")
            model_combo.set("")
    ...
    def load_config(self) -> bool:
        ...
        for r in rows:
            ...
            m = r.get("model")
            self.add_api(p, initial_model=m)
    ...
    def refresh_cache_stats(self):
        try:
            from pdf_engine.cache import GLOBAL_CACHE
        ...
        for item in self.api_rows:
            ...
            if on.get():
                ...
                if mod:
                    ref_model = mod
                    break
        if not ref_model and self.api_rows:
            mod = self.api_rows[0][4].get().strip()
            if mod:
                ref_model = mod
        if ref_model:
    ...
    def start(self):
        ...
        for item in self.api_rows:
            ...
            if on.get() and key.get().strip():
                ...
                mod = mv.get().strip()
                if mod:
                    keys.append(f"{prov}@{mod}:{key.get().strip()}")
                else:
