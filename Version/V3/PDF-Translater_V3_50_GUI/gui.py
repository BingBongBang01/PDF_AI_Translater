...
def _real_python_candidates():
    """Return real Python executables, excluding WindowsApps aliases."""
    found=[]
    def add(p):
        if not p: return
        try: p=str(Path(p).resolve())
        except Exception: p=str(p)
        if "WindowsApps" in p: return
        if Path(p).is_file() and p not in found: found.append(p)
    add(sys.executable)
    for cmd in ("py","python"):
        exe=_shutil.which(cmd)
        if exe and "WindowsApps" not in exe: add(exe)
    if os.name=="nt":
        import subprocess
        try:
        ...
        add(Path(local)/"Programs"/"Python"/"Python312"/"python.exe")
    return found

def _has_pymupdf(python_exe):
    try:
        r=subprocess.run([python_exe,"-c","import pymupdf"],
                         stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=15)
        return r.returncode==0
    except Exception:return False

def _ensure_correct_python():
    """If launched by WindowsApps Python, relaunch with the Python that owns PyMuPDF."""
    if getattr(sys,"frozen",False) or os.name!="nt": return
    current=str(sys.executable)
    if "WindowsApps" not in current:
        return
    for pyexe in _real_python_candidates():
        if pyexe != current and _has_pymupdf(pyexe):
            os.execv(pyexe,[pyexe,str(Path(__file__).resolve()),*sys.argv[1:]])
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PDF Translater v3.5")
