...
class App(tk.Tk):
    ...
    def build(self):
        ...
        self.npu_check=ttk.Checkbutton(n,text="NPU 사용",variable=self.use_npu)
        self.npu_check.grid(row=0,column=2,padx=5)
        self.gpu_check=ttk.Checkbutton(n,text="GPU 사용",variable=self.use_gpu)
        self.gpu_check.grid(row=0,column=3,padx=5)
        ttk.Label(n,text="모델").grid(row=0,column=4,padx=5)
        self.models=ttk.Combobox(n,textvariable=self.model,state="readonly",width=30)
        self.models.grid(row=0,column=5,padx=5); self.models.bind("<<ComboboxSelected>>",self.model_changed)
    ...
    def _install_python_worker(self) -> bool:
        ...
        if self._python312_ok():
            ...
            return True
        self._log("[Python] Python 3.12 미설치 -> winget으로 설치 시도")
        if _shutil.which("winget"):
            ...
            if code==0 and self._python312_ok():
                self._log("[Python] Python 3.12 설치 완료")
                return True
            ...
            return False
        self._log("[Python][경고] winget이 없어 자동 설치 불가. "
    ...
    def start(self):
        ...
        argv=[str(ENGINE),self.inp.get(),
              ...
              "--local-runtime",self.runtime.get()]
