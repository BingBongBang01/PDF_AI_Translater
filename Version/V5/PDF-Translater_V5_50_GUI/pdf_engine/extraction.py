...
def extract_segments(doc: "pymupdf.Document", page_filter: set[int] | None,
    ...
    for pno in range(doc.page_count):
        ...
        # 페이지 전체가 아니라 "말풍선(블록) 단위"로 개별 신뢰도를 판단한다 - 한 말풍선
        # 안의 통과율이 낮으면 그 말풍선만 버리고, 다른 말풍선은 그대로 살린다. 만화처럼
        # 페이지 전체가 노이즈인 경우는 모든 블록이 낮은 통과율을 보여 결과적으로 동일하게
        # 다 걸러진다(안전성은 그대로 유지).
