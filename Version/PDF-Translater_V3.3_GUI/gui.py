...
import contextlib, io, traceback, importlib
...
def _restart_as_admin():
    """Windows에서 python gui.py / EXE 모두 UAC 관리자 권한으로 재실행."""
    ...
    if getattr(sys, "frozen", False):
        ...
        exe = sys.executable
        params = " ".join([f'"{Path(__file__).resolve()}"'] + [f'"{x}"' for x in sys.argv[1:]])
    rc = ctypes.windll.shell32.ShellExecuteW(None, "runas", exe, params, str(Path.cwd()), 1)
    if rc > 32:
        raise SystemExit(0)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("PDF Translater v3.3")
